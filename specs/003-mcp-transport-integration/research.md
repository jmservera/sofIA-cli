...
**SDK scope (v0.1.28)**: The Copilot SDK focuses on the LLM conversation surface — creating sessions, streaming chat responses, and providing a `ToolDefinition` interface so that the host application can describe tools available to the LLM. It does **not** implement the MCP client protocol itself (spawn subprocess, send JSON-RPC, parse responses). The SDK's role is to route LLM-generated tool-call requests to the host application's registered handlers; the host is responsible for executing the actual tool calls.

**Implication**: There is no SDK method like `sdk.callMcpTool()` or `sdk.connectMcpServer()`. The pattern is:

1. Register tools via `SessionOptions.tools[]` (ToolDefinition objects).
2. The SDK emits tool-call events (`type: 'tool_call'`) when the LLM decides to use a tool.
3. The host application handles tool-call events and executes the real work — which in this case means calling an MCP server.
4. The host sends the tool result back to the SDK to continue the conversation.

**SDK value in this feature**: The SDK *is* useful for the web search case (`web.search` tool) — we already register it via `WEB_SEARCH_TOOL_DEFINITION` and handle calls in the conversation loop. For direct MCP tool calls made by adapters (GitHub adapter, Context7 enricher), the SDK is bypassed — adapters call `McpManager.callTool()` directly without going through the LLM conversation.

### Decision

**Build a custom MCP transport in `src/mcp/mcpTransport.ts`** for the stdio and HTTP protocol layers, since the SDK does not provide this. The SDK's tool-calling surface is retained for LLM-initiated tool calls (discovery enrichment, web search via conversation). Direct adapter calls (GitHub, Context7, Azure) bypass the SDK and call `McpManager.callTool()` directly.

**Rationale**: Aligns with existing codebase architecture where adapters already call `mcpManager.callTool()` directly. Building a minimal protocol client is safer than trying to shoehorn direct MCP calls through the LLM conversation loop.
...
- Use a third-party MCP client library (`@modelcontextprotocol/sdk`) — viable but adds a dependency. The MCP client protocol (JSON-RPC 2.0 over stdio/HTTP) is simple enough to implement directly, matching exactly what `.vscode/mcp.json` configurations need. Revisit in a later feature if the protocol surface grows.
...
interface McpTransport {
  callTool(toolName: string, args: Record<string, unknown>, timeoutMs: number): Promise<ToolCallResponse>;
  isConnected(): boolean;
...
| Topic | Decision |
|-------|----------|
| SDK MCP support | SDK does not provide MCP client; build `mcpTransport.ts` for stdio + HTTP |
